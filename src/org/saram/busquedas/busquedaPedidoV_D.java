 ...
 */
@WebServlet(name = "busquedaPedidoIngresadoV_D", description = "Principal de pedidos", urlPatterns = { "/busquedaPedidoV_D" })
public class busquedaPedidoV_D extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (usuario != null) {
			...
			List<COrder> co = cox.buscarTodosConProformaSinFactura(ad);
			CBPartner cb = new CBPartner();
			CBPartner_X cbx = new CBPartner_X();
			DecimalFormat df = new DecimalFormat("###,###,###.##");
			out.println("<head>"
					+ "<title>Pedidos por aprobar</title>"
			...
			if (co.size() != 0) {
				...
				for (COrder coi : co) {
					...
					out.println("<li>");
					out.println("<a href=\"busquedaProforma?ad_user_id=" + ad.getAd_user_id() + "&c_bpartner_id="
