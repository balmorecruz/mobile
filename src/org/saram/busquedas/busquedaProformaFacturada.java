...
import org.saram.accesos.CInvoice_X;
 ...
 */
@WebServlet(name = "busquedaProformaf", description = "Principal de pedidos", urlPatterns = { "/busquedaProformaf" })
public class busquedaProformaFacturada extends HttpServlet {
	 ...
	 */
	public busquedaProformaFacturada() {
		super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (usuario != null) {
			...
			System.out.println("TAMAÑO PROFORMAS:" + co.size());
			CBPartner cb = new CBPartner();
			CBPartner_X cbx = new CBPartner_X();
			DecimalFormat df = new DecimalFormat("###,###,###.##");
			out.println("<head>" + "<title>Proforma facturada</title>"
			...
			if (co != null) {
				if (co.size() != 0) {
					...
					for (COrder coi : co) {
						...
						ci = cix.buscarFacturadoPorOrden(coi.getC_order_id());

						out.println("<li>");
						out.println("<a href=\"pedido_m_v.jsp?ad_user_id=" + ad.getAd_user_id() + "&c_bpartner_id="
								+ coi.getC_bpartner_id() + "&c_order_id=" + coi.getC_order_id()
								+ "\" target=\"_self\">");
						...
						if (ci != null) {
							...
							fechaInvoice = new SimpleDateFormat("dd/MM/yyyy").format(ci.getDateacct());
							out.println("Fecha de facturacion: " + fechaInvoice);
							out.println("</div>");
						}
			...
			// out.println("</div>");
			out.println("<div class=\"toolbar\">");
			out.println("<a href=\"pedido?ad_user_id=" + usuario
					+ "\" class=blueButtonRegresar>Regresar</a><h1 id=\"pageTitle\">Por Aprobar</h1>");
			out.println("</div>");
			out.close();
		}
	}
