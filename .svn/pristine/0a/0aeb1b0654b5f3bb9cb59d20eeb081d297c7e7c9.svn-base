...
<body>
	<div class="toolbar">
	<a href="busquedaPedidos?c_order_id=<%=request.getParameter("c_order_id")%>&ad_user_id=<%=request.getParameter("ad_user_id")%>" class=blueButtonRegresar>Regresar</a>
		<h1>Mobile SARAM</h1>
	</div>

	<form id="screen1" title="Mobile Saram" class="panel" name="formname"
		action="${pageContext.request.contextPath}/busquedaCargos
		...
		<fieldset>
			<div class="row">
				<label>Nombre</label> <input type="hidden" name="ad_user_id"
					...
					type="text" name="description" style="text-transform: uppercase;"
					placeholder="LUGAR DESTINO">
			</div>
		<%--	<div class="row">
				<label>Codigo</label> <input type="text" name="mProductID"
					style="text-transform: uppercase;" placeholder="CODIGO DE PRODUCTO">
			</div>
			 --%>
			<input id="ad_user_id"
