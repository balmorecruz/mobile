...
<%!
private void precioProducto(javax.servlet.jsp.JspWriter out, String cb, String mp,String c_order_id) {
	try {
			...
			String sql = "select coalesce(sum(d.pricelist),0) " + "from " + "m_pricelist a " + "left join "
					...
					+ "where c.isactive = 'Y' and b.c_order_id =" + c_order_id + " and d.m_product_id = " + mp;
...
<%
	precioProducto(out, request.getParameter("c_bpartner_id"), request.getParameter("m_product_id"),request.getParameter("c_order_id"));
%>
<body>
	<form title="Linea" method="post" selected="true" class="panel"
		action="validaproducto?c_order_id=<%=request.getParameter("c_order_id")%>
		&modo=3&c_bpartner_id=<%=request.getParameter("c_bpartner_id")%>"
...
-->
			...
			<div id="formulario2">
				<div class="row">
					<label id="QtyEnteredL"
						...
						name="QtyEntered" maxlength="22" value=" "
				...
				<div class="row">
					...
					<div class="fieldValue">
						<input class="Cmandatory" id="netolinea" name="netolinea"
							value="0" disabled="true" type="number" />
					</div>
				</div>
	...
	<div class="toolbar">
		<a
			href="busquedaProducto.jsp?ad_user_id=<%=request.getParameter("ad_user_id")%>&&c_bpartner_id=<%=request.getParameter("c_bpartner_id")%>&&c_order_id=<%=request.getParameter("c_order_id")%>"
