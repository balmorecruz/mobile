...
<%!
private void precioProducto(javax.servlet.jsp.JspWriter out, String ccid) {
	System.out.println("HOLAAAA fi");	
	try {
			CCharge c = new CCharge();
			CCharge_X  cx = new CCharge_X();
			List<CCharge> d = cx.buscarUnoByID(ccid);
			if (d.get(0) == null) {
				precio = new Float(0.0);
			} else {
				d.get(0).getChargeamt();
				precio = d.get(0).getChargeamt();
			}
		} catch (Exception e1) {
...
<%
	precioProducto(out, request.getParameter("c_charge_id"));
%>
<body>
		...
		<fieldset>
			...
			<div class="row">
				<label id="M_Product_IDL" for="M_Product_IDF"
					...
					value="<%=request.getParameter("c_charge_id")%> - <%=request.getParameter("m_product_name")%>"
...
-->
		...
		<fieldset>
			<div class="row">
				<label id="QtyEnteredL"
					...
					name="QtyEntered" maxlength="22" value="1"
			...
			<div class="row">
				...
				<div class="fieldValue">
					<input class="Cmandatory" id="netolinea" name="netolinea" value="0"
						disabled="true" type="number" />
				</div>
			</div>
			<!-- <div class="row">
	...
	<div class="toolbar">
	<a href="busquedaPedidos?ad_user_id=<%=request.getParameter("ad_user_id")%>&&c_bpartner_id=<%=request.getParameter("c_bpartner_id")%>&&c_order_id=<%=request.getParameter("c_order_id")%>" class=blueButtonRegresar>Regresar</a>
		<h1 id="pageTitle"></h1>
