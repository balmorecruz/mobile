...
<body>
	...
	<form id="screen1" title="Pedido" class="panel" name="formname"
		action="busquedaPedidos?c_order_id=<%=request.getParameter("c_order_id")%>&c_bpartner_id=<%=request.getParameter("c_bpartner_id")%>"
		...
		<fieldset>
			...
			<div class="row">
				<label id="C_BPartner_Location_IDL" for="C_BPartner_Location_IDF"
					...
					<select disabled="true" name="c_bpartner_location_id"
		...
		<div>
			<input type="submit" class="whiteButton" name="lineapedido"
				value="SELECCIONAR PRODUCTOS" id="btnLeft" /> <!-- <input
		...
		try {
		//	if (!value.isEmpty()) {
				if (value != null && cb.getSo_creditlimit()!=0) {
					Float val = value;
					if (val <= 0){
						out.println("<especial id='limitecredito'><em>"
								+"El limite de crédito aprobado del cliente es: $"+cb.getSo_creditlimit() 
								+ "</br></br>Límite de crédito de cliente excedido, incluyendo nuevo pedido, por: $"
								+ (Math.abs(val) + grandTotal) + " </br></br>" + "Monto vencido de: $" + Math.abs(val)
								+ " </br></br> " + "Total a pagar para solventar estatus de cuentas por cobrar: $"
								+ Math.abs(val) + "</em></especial></br>");
						out.println("<p><n> Comentarios</n> </p>");
						out.println("<textarea name=\"aprobarcredito\" rows=\"10\" cols=\"50\"></textarea>");
						out.println("<input type=\"submit\" class=\"redButton\" id=\"solicitar\" name=\"solicitar\" value=\"SOLICITAR APROBACION\" id=\"btnLeft\" />");
